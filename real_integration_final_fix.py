# real_integration_final_fix.py - Final Fixed Version
...
# Gemini + Smart Analysis Service (FIXED)
...
{{"viral_potential": 8, "content_saturation": 6, "audience_interest": 9, "monetization_opportunity": 7, "content_angles": ["tutorial", "review", "analysis"], "reasoning": "การวิเคราะห์สั้นๆ"}}"""
    ...
    def create_advanced_smart_analysis(self, trend_topic: str, popularity_score: int) -> AIAnalysisResult:
        ...
        category_scores = {
            ...
            "entertainment": {
                "keywords": ['หนัง', 'movie', 'ตัวอย่าง', 'trailer', 'ซีรี่ส', 'drama', 'netflix', 'anaconda', 'official', 'chief', 'war'],
            ...
            "music": {
                "keywords": ['เพลง', 'music', 'ft.', 'official', 'mv', 'song', 'artist', 'นางฟ้า', 'ธนากร', 'time', 'flies', 'jeff', 'satur'],
            ...
            "lifestyle": {
                "keywords": ['อาหาร', 'food', 'travel', 'beauty', 'fashion', 'health', 'fitness', 'บิวตี้', 'ฟิตเนส', 'ไลฟ์สไตล์'],
...
class FixedDatabaseService:
    ...
    def save_opportunities(self, opportunities: List[Dict[str, Any]]):
        """FIXED: Save content opportunities to database"""
        try:
            if "sqlite" in self.database_url:
                db_path = self.database_url.replace("sqlite:///", "")
                conn = sqlite3.connect(db_path)
                ...
                for opp in opportunities:
                    ...
                    if analysis:
                        try:
                            # FIXED: Convert content_angle properly
...
class FinalPipeline:
    def __init__(self, config: Dict[str, str]):
        self.config = config
        self.db_service = FixedDatabaseService(config["database_url"])
        self.stats = {
    ...
    async def run_full_pipeline(self) -> Dict[str, Any]:
        ...
        print("Starting FINAL Fixed Pipeline")
        ...
        try:
            ...
            async with GeminiSmartAnalysisService(self.config.get("gemini_api_key", "")) as ai_service:
                ...
                for i, trend_data in enumerate(trends_to_analyze):
                    try:
                        ...
                        if analysis:
                            # FIXED: Handle content_angle properly
                            best_angle = analysis.content_angles[0] if analysis.content_angles else "General content"
                            
                            opportunity = {
                                "trend_data": trend_data,
                                "analysis": analysis,
                                "content_angle": best_angle,  # Now it's a string
                                "content_plan": None
                            }
            ...
            print(f"\nFINAL Fixed pipeline completed in {duration:.1f}s")
            ...
            # Show top opportunities (FIXED string slicing)
            if ai_opportunities:
                print(f"\nTop opportunities:")
                ...
                for i, opp in enumerate(sorted_opportunities[:5]):
                    if opp.get("analysis"):
                        ...
                        angle_display = str(opp['content_angle'])[:50] + "..." if len(str(opp['content_angle'])) > 50 else str(opp['content_angle'])
                        reasoning_display = str(analysis.reasoning)[:60] + "..." if len(str(analysis.reasoning)) > 60 else str(analysis.reasoning)
                        
                        print(f"   {i+1}. {topic_display}")
                        print(f"      Angle: {angle_display}")
                        print(f"      Score: {analysis.overall_score:.1f}/10")
                        print(f"      Viral Potential: {analysis.viral_potential}/10")
                        print(f"      Analysis: {reasoning_display}")
            
            logger.info("FINAL Fixed pipeline completed successfully")
...
async def main():
    """Main function"""
    print("AI Content Factory - FINAL FIXED Version")
    print("=" * 60)
    ...
    services.append("Smart Analysis")
    services.append("Google Trends")
    ...
    results_file = f"final_pipeline_results_{timestamp}.json"
    ...
    # Final summary
    if results["success"]:
        print(f"\nFINAL Fixed Integration completed successfully!")
